...
def sogl_upd(FIO, EXECUTOR_ID, tg_id):
    ...
    if count_doc > 0:
        ...
        for i in range(len(documents)):
            p = Process(target=sogl_response, args=(s, documents[i], DNSID, queue, FIO))
            p.start()
            processes.append(p)
        ...
        df_final.to_excel(os.path.join(os.getcwd(), 'export_deadlines.xlsx'))
        # df_final.drop(['Дата согла', 'Адресат', 'doc_id', 'link_id'], axis=1, inplace=True)
        template = 'https://mosedo.mos.ru/document.card.php?id='
        if not df_final.empty:
            ...
            if not df_deadline_today.empty:
                ...
                deadline_id = df_deadline_today['doc_id'].to_list()
                for i in range(len(deadline_line)):
                    href_new = template + str(deadline_id[i])
                    deadline_line[i] = str(i + 1) + ') ' + f"<a href='{href_new}'>{deadline_line[i]}</a>"
            ...
            if not df_final_today.empty:
                ...
                deadline_id = df_final_today['doc_id'].to_list()
                for i in range(len(lst_line_today)):
                    href_new = template + str(deadline_id[i])
                    lst_line_today[i] = str(i + 1) + ') ' + f"<a href='{href_new}'>{lst_line_today[i]}</a>"
            ...
            if not df_final_older.empty:
                ...
                for i in range(len(lst_line_today)):
                    href_new = template + str(deadline_id[i])
                    lst_line_today[i] = str(i + 1) + ') ' + f"<a href='{href_new}'>{lst_line_today[i]}</a>"
                text_today = df_final_older['Краткое содержание'].to_list()
...
def sogl_response(s, doc_number, DNSID, queue, fio):
    ...
    df = pd.DataFrame(datata, columns=['na nomer', 'link_id', 'doc_id', 'срок РГ', 'Исполнитель'])
    queue.put(df)


def deadline_check(s, DNSID, doc_id, fio):
    warnings.filterwarnings('ignore')
            ...
            if not item.parent.find('div', class_='resolution-item__vzamen'):
                ...
                if item.find('span'):
                    if item.find('span').find('span'):
                        if fio in item.find('span').find('span').text:
    ...
    print(df)
    df_RG = df[df['A'].str.contains(fio.split(' ')[0])]
    ...
    if 'Документ ДСП' not in executor:
        ...
        ###################################################################################################################################################
        data = [srok_rg, executor_str]
    else:
        data = [srok_rg, 'Документ ДСП']
